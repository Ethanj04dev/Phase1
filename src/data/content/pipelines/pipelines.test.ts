...
import { PREPARATION_DOMAINS, preparationDomain } from '@/domain/pipeline/domains';
import { isVerified, type Verified } from '@/domain/pipeline/provenance';
import { domainWeightsSumToOne, findDomain } from '@/domain/pipeline/types';

import { findPipeline, hasPipelineDefinition, PARARESCUE, RANGER, SEAL, PIPELINES } from './index';
...
describe('target catalog', () => {
  it('finds a modelled target and reports an unmodelled one honestly', () => {
    expect(findPipeline('pararescue')).toBe(PARARESCUE);
    expect(hasPipelineDefinition('pararescue')).toBe(true);
    expect(findPipeline('swcc')).toBeUndefined();
    expect(hasPipelineDefinition('swcc')).toBe(false);
  });

  it.each(PIPELINES.map((t) => [t.id, t] as const))('%s has unique domain ids', (_id, target) => {
    const ids = target.domains.map((d) => d.id);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it.each(PIPELINES.map((t) => [t.id, t] as const))(
...
describe('official data is never fabricated', () => {
  ...
  it.each(PIPELINES.map((t) => [t.id, t] as const))(
    ...
    (_id, target) => {
      for (const benchmark of target.preparationBenchmarks) {
...
describe('water safety', () => {
  ...
  const waterTargets = PIPELINES.filter(
...
describe('seal is the swim-dominant profile', () => {
  it('puts more combined weight in the water than any other target', () => {
    const waterWeight = (target: (typeof PIPELINES)[number]): number =>
  ...
  it('sets the hardest swim benchmark in the catalog', () => {
    const swimTargets = PIPELINES.flatMap((target) => {
      const benchmark = target.preparationBenchmarks.find((b) => b.eventId === 'swim_500m');
      return benchmark ? [benchmark.target] : [];
    });
    const seal = SEAL.preparationBenchmarks.find((b) => b.eventId === 'swim_500m');
