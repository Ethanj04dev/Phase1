...
} from '@/domain/pipeline/milestones';
import type { MilestoneDefinition, PipelineDefinition } from '@/domain/pipeline/types';
...
export interface MilestonesView {
  target: PipelineDefinition | null;
...
export function useMilestones(): UseMilestones {
  ...
  const fetcher = useCallback(async (): Promise<Result<MilestonesView>> => {
    ...
    const target = findPipeline(profile.goalId) ?? null;
    if (!target) {
      return ok({
        target: null,
        standings: [],
