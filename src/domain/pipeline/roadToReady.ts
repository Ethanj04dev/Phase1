...
import type { PipelineReadiness } from '@/domain/readiness/pipelineScore';

import { preparationDomain, type PreparationDomainId } from './domains';
import { preparationBenchmarkFor, type PipelineDefinition, type PipelineDomain } from './types';
...
function eventGaps(
  ...
  return domain.eventIds.flatMap((eventId): EventGap[] => {
    ...
    const benchmark = preparationBenchmarkFor(target, eventId);
