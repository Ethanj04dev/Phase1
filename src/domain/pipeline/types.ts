...
// --- Standards --------------------------------------------------------------
//
// OfficialStandard and PreparationBenchmark are separate types with different field
...
export interface ProficiencySkill {
  ...
  suggestedTarget: ProficiencyLevel;
  /**
...
export interface PipelineDefinition {
  ...
  domains: readonly PipelineDomain[];
  officialStandards: readonly OfficialStandard[];
  preparationBenchmarks: readonly PreparationBenchmark[];
  assessments: readonly PipelineAssessment[];
...
export function findDomain(
  target: PipelineDefinition,
  domainId: PreparationDomainId,
): PipelineDomain | undefined {
  return target.domains.find((domain) => domain.id === domainId);
}

export function preparationBenchmarkFor(
  target: PipelineDefinition,
  eventId: AssessmentEventId,
): PreparationBenchmark | undefined {
  return target.preparationBenchmarks.find((benchmark) => benchmark.eventId === eventId);
}
