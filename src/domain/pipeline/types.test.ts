...
function target(overrides: Partial<PipelineDefinition> = {}): PipelineDefinition {
  return {
    id: 'test_target',
    name: 'Test Target',
    shortName: 'TEST',
...
describe('official standards and Zero Phase benchmarks stay apart', () => {
  ...
  const benchmark: PreparationBenchmark = {
    eventId: 'run_1_5_mile',
    target: 570,
  ...
  const populated = target({
    ...
    preparationBenchmarks: [benchmark],
  });

  it('reads each from its own field', () => {
    expect(officialStandardFor(populated, 'run_1_5_mile')).toBe(standard);
    expect(preparationBenchmarkFor(populated, 'run_1_5_mile')).toBe(benchmark);
  });
  ...
  it('returns nothing for an event the target does not cover', () => {
    ...
    expect(preparationBenchmarkFor(populated, 'pull_ups')).toBeUndefined();
  });
});
