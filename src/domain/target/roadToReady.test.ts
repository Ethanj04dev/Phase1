...
import { PARARESCUE } from '@/data/content/targets';
import { calculateTargetReadiness } from '@/domain/readiness/targetScore';
...
function roadFor(results: readonly AssessmentResult[]) {
  const readiness = calculateTargetReadiness(PARARESCUE, { results });
...
describe('road to ready', () => {
  ...
  it('agrees with the readiness engine about the priority domain', () => {
    const readiness = calculateTargetReadiness(PARARESCUE, { results: SWIM_WEAK });
...
describe('nothing left to do', () => {
  ...
  it('marks domains at the benchmark as maintain, with no impact', () => {
    const road = buildRoadToReady(
      PARARESCUE,
      calculateTargetReadiness(PARARESCUE, { results: STRONG }),
      STRONG,
    );
