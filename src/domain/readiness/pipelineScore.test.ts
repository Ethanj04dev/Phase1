...
import type { PipelineDefinition, PipelineDomain } from '@/domain/pipeline/types';
import { PERFORMANCE_CATEGORIES } from '@/domain/types';

import { calculateReadiness } from './score';
import { calculatePipelineReadiness, scoreDomain } from './pipelineScore';
 ...
 */
function pipelineFromGoal(goal: Goal): PipelineDefinition {
  const domains: PipelineDomain[] = PERFORMANCE_CATEGORIES.map((category) => ({
...
describe('equivalence with the category engine', () => {
  it.each(GOALS.map((goal) => [goal.id, goal] as const))(
    ...
    (_id, goal) => {
      ...
      const after = calculatePipelineReadiness(pipelineFromGoal(goal), {
  ...
  it('agrees on coverage when only part of the battery is tested', () => {
    ...
    const after = calculatePipelineReadiness(pipelineFromGoal(goal), { results: partial });
  ...
  it('agrees that nothing tested means no score', () => {
    const goal = getGoalOrDefault('pararescue');
    expect(calculatePipelineReadiness(pipelineFromGoal(goal), { results: [] })).toBeNull();
...
function domain(overrides: Partial<PipelineDomain> & { id: PreparationDomainId }): PipelineDomain {
  return {
    weight: 1,
    demand: 'high',
    rationale: 'x',
...
function targetWith(domains: PipelineDomain[]): PipelineDefinition {
  return {
    id: 't',
    name: 'T',
    shortName: 'T',
...
describe('Target-specific domains', () => {
  ...
  it('scores only the domains the Target defines', () => {
    ...
    const readiness = calculatePipelineReadiness(runnerOnly, { results: FULL_BATTERY });
  ...
  it('never scores a domain the Target omits, however much data exists', () => {
    ...
    const readiness = calculatePipelineReadiness(noSwimming, { results: FULL_BATTERY });
...
describe('mixed measurement Target', () => {
  ...
  it('combines performance, proficiency and behavioural domains', () => {
    const readiness = calculatePipelineReadiness(pjLike, {
  ...
  it('is deterministic', () => {
    ...
    expect(calculatePipelineReadiness(pjLike, input)).toEqual(
      calculatePipelineReadiness(pjLike, input),
    );
  });
});
