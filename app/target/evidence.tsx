    ...
    >
      <AsyncBoundary state={state} onRetry={reload}>
        {({ target, readiness, results }) => {
          ...
          const measured = target.domains.filter(
            (domain) => readiness.domains[domain.id] !== undefined,
          );
          const unmeasured = target.domains.filter(
            (domain) => readiness.domains[domain.id] === undefined,
          );
            ...
            <>
              ...
              <View>
                ...
                <Card style={{ gap: theme.spacing.sm }}>
                  <Text variant="bodySm" color="textSecondary">
                    {`The weighted points sum to ${weightedTotal.toFixed(1)}. The measured domains cover ${formatPercent(coveredWeight)} of this target's weighting, so the sum is divided by ${coveredWeight.toFixed(2)} rather than by 1 — you are scored on what was measured, not punished with zeros for what was not.`}
                  </Text>
