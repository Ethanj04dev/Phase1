...
import type { Source } from '@/domain/pipeline/provenance';
import type { IntelArticle, IntelCategory, PipelineDefinition } from '@/domain/pipeline/types';
 ...
 */
function sourcesFor(pipeline: PipelineDefinition, article: IntelArticle): readonly Source[] {
  return (article.sourceIds ?? []).flatMap((id) => {
    const source = pipeline.sources.find((candidate) => candidate.id === id);
  ...
  return (
    ...
    >
      <AsyncBoundary state={state} onRetry={reload}>
        {({ pipeline }) => {
          if (!pipeline || pipeline.intel.length === 0) {
            return (
              <Text variant="body" color="textSecondary">
                Career information is not yet available for your pipeline.
              </Text>
            );
          }
              ...
              {pipeline.intel.map((article) => (
                <View key={article.id} style={{ gap: theme.spacing.md }}>
                  ...
                  <Card style={{ gap: theme.spacing.md }}>
                    ...
                    {sourcesFor(pipeline, article).map((source) => (
