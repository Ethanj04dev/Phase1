...
import { verifiedValue } from '@/domain/pipeline/provenance';
import {
  officialStandardFor,
  preparationBenchmarkFor,
 ...
 */
function measurementNote(domain: PipelineDomain): string {
...
function EventRow({
  ...
  const theme = useTheme();
  const benchmark = preparationBenchmarkFor(target, event.id);
        ...
        {({ pipeline, results }) => {
          if (!pipeline) {
            return (
              <Text variant="body" color="textSecondary">
                Fitness standards are not yet available for your pipeline.
              </Text>
            );
          }
          ...
          return (
            <>
              ...
              {pipeline.domains.some((domain) => domain.eventIds.length === 0) ? (
