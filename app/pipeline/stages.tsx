...
import { verifiedValue, VERIFICATION_REQUIRED } from '@/domain/pipeline/provenance';
  ...
  return (
    ...
    >
      <AsyncBoundary state={state} onRetry={reload}>
        {({ pipeline }) => {
          if (!pipeline || pipeline.pipeline.length === 0) {
            return (
              <Text variant="body" color="textSecondary">
                Pipeline information is not yet available for your pipeline.
              </Text>
            );
          }

          const anyPlaceholder = pipeline.pipeline.some((stage) => stage.isPlaceholder);
            ...
            <>
              ...
              <View>
                {pipeline.pipeline.map((stage, index) => {
                  const isLast = index === pipeline.pipeline.length - 1;
