...
import type { PipelineDomain } from '@/domain/pipeline/types';
    ...
    <View style={{ padding: theme.spacing.lg, gap: theme.spacing.md }}>
      ...
      <View style={{ gap: theme.spacing.xxs }}>
        <Text variant="caption" color="textTertiary">
          {`Suggested by Zero Phase: ${PROFICIENCY_LEVEL_LABELS[skill.suggestedTarget]}`}
        </Text>
        <Text variant="caption" color="textTertiary">
  ...
  return (
    ...
    >
      <AsyncBoundary state={state} onRetry={reload}>
        {({ pipeline, profile, ratings }) => {
          const domains = (pipeline?.domains ?? []).filter(
          ...
          if (domains.length === 0) {
            return (
              <Text variant="body" color="textSecondary">
                Your pipeline has no skill-rated areas.
              </Text>
            );
          }
